extend class Lupo3DWeapon
{
	action void Lupo3D_WeaponReady(int weapflags = 0)
	{
	...
	/*
	action void Lupo3D_LowAmmoSoundWarning(string type = "default", string ammoItem = "SuperMario64ForTheXboxOne")
	{
		Ammo count;
		...
		if(!count)
		{
			console.printf("Lupo3D_LowAmmoSoundWarning: Invalid ammo class");
			return;
		}
		...
		{
			double clickvolume = Lupo3D_Math.LinearMap(currentammocount, 1.0, lowammocount, 1.0, 0.5);
			...
			if(currentammocount == 0)
				console.printf("Lupo3D_LowAmmoSoundWarning: Ammo count is 0 instead of 1, was the function called after the ammo item was taken?");
		}
	}
	*/
	//- L, 0 C, + R
	// [gng] credits to jaih1r0 for the position calculation, modified by me to fix the pitch bug
	Action void Lupo3D_GunSmoke(double d1 = 0, double d2 = 0 , double d3 = 0, string SActor = "Lupo3D_GunFireSmoke")
	{
		double PVZ;
		Lupo3DPlayer plr = Lupo3DPlayer(invoker.owner);
		...
		Lupo3D_GunFireSmoke Smoke = Lupo3D_GunFireSmoke(Spawn(SActor, spos));
		
		If(Smoke)
		{
			Smoke.master = invoker.owner;
			//Smoke.Vel = Lupo3D_Math.AngleToVector3D(self.Angle, -self.Pitch);
	...
	action void Lupo3D_RaiseWeapon() {
		A_Raise(25);
	}
	
	action void Lupo3D_LowerWeapon() {
		A_Lower(25);
	}
	/*
	...
	{
        return (owner.CountInv("Lupo3D_PowerStrength") >= 1);
	}
	
	//Adds weapon recoil, modifying it if the owner has a berserk powerup
	action void Lupo3D_WeaponRecoil(float pitchDelta, float angleDelta, float powerMod = 0.5)
	{
        if (invoker.OwnerHasBerserk())
            Lupo3D_WeaponRecoilBasic(pitchDelta * powerMod, angleDelta * powerMod);
        else
            Lupo3D_WeaponRecoilBasic(pitchDelta, angleDelta);
	}
	*/
	//Normally you want to use PB_WeaponRecoil instead, because it accounts for the berserk powerup
	action void Lupo3D_WeaponRecoilBasic(float pitchDelta, float angleDelta = 0)
	{
		double fac = 1.0;
	...
	Action State Lupo3D_SpawnCasing(String Missile,Double ShellX,Double ShellY,Double ShellZ,Double ShellV_X,Double ShellV_Y,Double ShellV_Z)
	{
	...
	action void Lupo3D_AmmoIntoMag(String AmmoMag_Action,String AmmoPool_Action,int MagazineMaxFill_Action, int takeReserve)
		{
	...
	action void Lupo3D_UnloadMag(String AmmoMag_Action,String AmmoPool_Action, int giveReserve)
	{
	...
	action void Lupo3D_FireBullets(string type,int amount,float angle,double offs,double height,float pitch)
	{
	...
	action void Lupo3D_QuakeCamera(int qDur, float camRoll)
	{
	...
	action void Lupo3D_HandleWeaponFeedback(int qDur, float camRoll, float pitchDelta, float angleDelta, double d1 = 0, double d2 = 0 , double d3 = 0)
	{
		Lupo3D_QuakeCamera(qDur, camRoll);
		Lupo3D_WeaponRecoilBasic(pitchDelta, angleDelta);
		Lupo3D_GunSmoke(d1, d2, d3);
	}
	
	action void Lupo3D_HandleCrosshair(int num)
	{
        CVar crosshair_settings = CVar.FindCVar('Lupo3D_weapon_crosshairs');
